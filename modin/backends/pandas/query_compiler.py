...
class PandasQueryCompiler(BaseQueryCompiler):
    ...
    def merge(self, right, **kwargs):
        ...
        if how in ["left", "inner"] and left_index is False and right_index is False:
            ...
            right_cylon_frame = right._modin_frame
            new_self = self._modin_frame.merge(right_cylon_frame, **kwargs)  # PandasOnPyCylonFrame.merge(right_cylon_frame)
            new_self = self.__constructor__(new_self)

            is_reset_index = True


            # modifications:sort, reset index done to merged result here in compiler.'''
            # Since currently in cylon, column names are not preserved after merge, shall put manually from which column to sort
            on = 'rt-4'
    ...
    #original merge in modin
    def merge_modin(self, right, **kwargs):
        """
        Merge DataFrame or named Series objects with a database-style join.
